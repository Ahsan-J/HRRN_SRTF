...
function frame(){
    if(Process[i].Pro_PC<Process[i].Pro_Ins){
        ...
        var waitingTime = quantum-Process[j].Pro_Arrival;
        Process[j].HRRN_val = (waitingTime+Process[j].Pro_Ins)/Process[j].Pro_Ins;
...
function appendProcess_Table2(i){
    ...
    var waitingTime = quantum-Process[i].Pro_Arrival;
    Process[i].HRRN_val = (waitingTime+Process[i].Pro_Ins)/Process[i].Pro_Ins;
